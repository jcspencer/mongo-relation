...
Schema.prototype._addRelationship = function (type, model, options) {
  ...
  var castOpts = { type: ObjectId, index: true, ref:  model }
  castOpts.required = options.required  || false;
  var cast = array ? [ castOpts ] : castOpts;
  pathName = options.through || (array ? pluralize(model.toLowerCase()) : model.toLowerCase());
  ...
  this.paths[pathName].options.dependent = options.dependent || false;
  this.paths[pathName].options.setParent = true;
  if (type == 'hasMany' && options.hasOwnProperty('setParent')) {
    this.paths[pathName].options.setParent = options.setParent;
  }
  return this;
};
/**
