...
Schema.prototype._addRelationship = function(type, model, options) {
    ...
    var array = type === "hasMany" || type === "habtm"
            , pathName
            , cast = array
            ? [{type: ObjectId, index: true, ref: model}]
            : {type: ObjectId, index: true, ref: model};
    ...
    if (options.mappedBy && options.model) {
        this.paths[pathName].options.mappedBy = options.mappedBy;
        this.paths[pathName].options.model = options.model;
    }
    ...
    for (var path in this.paths) {
        ...
        if (type) {
            if (options[type] && options[type] === modelName) {
                return {type: type, name: path};
                break;
            }
        } else if (options.belongsTo === modelName || options.habtm === modelName) {
            var type = Array.isArray(options.type)
                    ? "habtm"
                    : "belongsTo"
            return {type: type, name: path};
            break;
...
MongooseArray.prototype._getRelationship = function() {
    var schemaOpts = this._schema.options
            , type
            , ref
            , options = {};
    ...
    return {type: type, ref: ref, options: options};
};
...
MongooseArray.prototype.build = function(objs) {
    ...
    var self = this
            , parent = this._parent
            , childModelName = this._schema.options.hasMany || this._schema.options.habtm
...
MongooseArray.prototype.find = function(conditions, fields, options, callback) {
    ...
    var parent = this._parent
            , childModel = parent.model(this._schema.options.hasMany || this._schema.options.habtm)
...
MongooseArray.prototype.populate = function(fields, callback) {
    ...
    var parent = this._parent
            ...
            , path = this._path
            , self = this
            , schemaOpts = this._schema.options;

    if (schemaOpts.mappedBy) {
        var model = schemaOpts.model, mappedBy = schemaOpts.mappedBy, queryObj = {};
        queryObj[mappedBy] = parent.id;
        model.where(mappedBy).equals(parent._id).exec(function(err, children) {
            if (fields) {
                var fns = [];
                ...
                for (var child in children) {
                    for (var field in fields) {
                        var chain = fields[field].split('.');
                        if (chain.length > 1) {
                            var subfns = [];
                            var startLink = chain.splice(0, 1);
                            subfns.push(function(callback) {
                                children[child].populate(startLink, function(err, child) {
                                    callback(null, child, child[startLink]);
                                });
                            });                          
                            chain.map(function(link) {
                                subfns.push(function(parent, sub, callback) {
                                    sub.populate(link, function(err, sub) {
                                        callback(null, parent, sub[link]);
                                    });
                                });
                            });
                            fns.push(function(callback) {
                                async.waterfall(subfns, function(err, results){
                                    callback(err, results);
                                });
                            });
                        } else {
                            fns.push(function(callback) {
                                children[child].populate(fields[field], function(err, child) {
                                    callback(null, child);
                                });
                            });
                        }
                ...
                async.parallel(fns, function(err, children) {
...
MongooseArray.prototype.remove =
        MongooseArray.prototype.delete = function(id, callback) {
    var args = arguments
